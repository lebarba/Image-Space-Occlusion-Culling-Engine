...
namespace Examples.OcclusionMap.Cacic
{
    ...
    public class DemoFpsSplitScreen : TgcExample
    {
        ...
        int frustumCulledMeshes;
        int occlusionCulledMeshes;
        ...
        private void drawDepthBuffer(Device d3dDevice)
        {
            ...
            //Frustum %
            //float fruPercent = (float)frustumCulledMeshes / occludees.Count * 100;
            //GuiController.Instance.Text3d.drawText("Frustum cull: " + string.Format(k"{0:0.##}", fruPercent) + "%", 0, 20, Color.LawnGreen);

            //Frustum %
            //float fruPercent = (float)(occludees.Count-frustumCulledMeshes) / occludees.Count * 100;
            //GuiController.Instance.Text3d.drawText(string.Format("Frustum culled {0}/{1}  ({2}) %", occludees.Count - frustumCulledMeshes,  occludees.Count, string.Format("{0:0.##}", fruPercent)), 0, 20, Color.LawnGreen);

            //Occlusion %
            //float occPercent = (float)occlusionCulledMeshes / occludees.Count * 100;
            //GuiController.Instance.Text3d.drawText("Occlusion cull: " + string.Format("{0:0.##}", occPercent) + "%", 0, 40, Color.LawnGreen);

            //float occPercent = (float)(occludees.Count - occlusionCulledMeshes) / occludees.Count * 100;
            //GuiController.Instance.Text3d.drawText(string.Format("Occlusion culled {0}/{1}  ({2}) %", occludees.Count - occlusionCulledMeshes, occludees.Count, string.Format("{0:0.##}", occPercent)), 0, 40, Color.LawnGreen);

            float occPercent = (float)(occlusionCulledMeshes) / (occludees.Count-frustumCulledMeshes) * 100;
            GuiController.Instance.Text3d.drawText(string.Format("Occlusion culling: {0} of {1}. ({2}) %", occlusionCulledMeshes, (occludees.Count - frustumCulledMeshes), string.Format("{0:0.##}", occPercent)), 0, 40, Color.LawnGreen);
