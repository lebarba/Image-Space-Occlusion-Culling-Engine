...
namespace Examples.OcclusionMap.Cacic
{
    ...
    /// </summary>
    public class DemoCacicThirdPersonFrustum : TgcExample
    {
        ...
        public override void init()
        {
            ...
            //Cargar occluders de ciudad
            TgcSceneLoader loaderOccluders = new TgcSceneLoader();
            TgcScene sceneOcc = loaderOccluders.loadSceneFromFile(GuiController.Instance.ExamplesMediaDir + "ModelosTgc\\CiudadGrandeCerradaOccluders\\CiudadGrandeCerradaOccluders-TgcScene.xml");
            for (int i = 0; i < sceneOcc.Meshes.Count; i++)
            {
                TgcMesh mesh = sceneOcc.Meshes[i];
                Occluder oc = new Occluder();
                oc.Mesh = mesh;
                occluders.Add(oc);
            }
            */
            ...
            GuiController.Instance.Modifiers.addBoolean("showOcclusionCull", "showOcclusionCull", false);
        ...
        private void drawThirdPersonScene(Device d3dDevice)
        {
            ...
            //Visible %
            float visPercent = 100f - fruPercent - occPercent;
            Color visColor = visPercent > 30f ? Color.OrangeRed : Color.LawnGreen;
            GuiController.Instance.Text3d.drawText("Visible: " + string.Format("{0:0.##}", visPercent) + "%", 0, 60, visColor);
